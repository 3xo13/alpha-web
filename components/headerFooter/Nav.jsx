...
const Nav = () => {
    ...
    return (
        <div className="overflow-x-hidden" key={uuidv4()}>
            <Cart show={showCart} set={setShowCart}/>

            <header className="">
                <div key={uuidv4()} className="flex  w-screen  pt-5 justify-between ">
                    <div className="w-1/3 h-full flex flex-row items-center justify-between">
                        {/* this is the cart btn */}
                        <button onClick={() => setShowCart(!showCart)} className='lg:ml-14 ml-5 mt-8 lg:mt-3'>
                            <img
                    ...
                    <div key={uuidv4()} className="flex justify-center items-center w-1/3 ">
                        {/* this is the logo */}
                        <Link href="/" className="flex gap-2 flex-col lg:flex-row items-center justify-end mr-7">
                            <img className="w-18 h-16"
                                src="/assets/images/logo.png"
                                alt="Alpha limit Logo"
                                />
                            <p className="text-xl lg:h-0.5 mb-5 font-bold uppercase  ">Alpha Limit</p>
                        </Link>
                    </div>
                    <div key={uuidv4()} className="w-1/3  flex justify-end ">

                        <nav
                            // this is the menu on desktop screens
                            className={` flex-row mr-10
                             gap-5 items-end pr-5 justify-evenly 
                            ...
                            <Link href="/contact" key={uuidv4()}>
                                <p className="menu_link_text">Contact</p>
                            </Link>
